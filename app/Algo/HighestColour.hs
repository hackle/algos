...
highestColour colours adjacents =
    let (State runSt) = solve outs coloursIndexed
        (final, st1) = runSt st
    in final
    where
        coloursIndexed = M.fromList $ zip [0..] colours
        outs = M.fromListWith (++) $ fmap (\[x, y] -> (x, [y])) adjacents
        ins = M.fromListWith S.union $ fmap (\[x, y] -> (y, S.singleton x)) adjacents
        loose =  M.filterWithKey (\k _ -> not $ M.member k ins) outs
        st = CState {
            _ins = ins,
            _loose = loose,
            _counts = M.mapWithKey (\k _ -> M.singleton (coloursIndexed M.! k) 1) loose }
...
-- try to get rid of the lambda (f, v) -> f v)
-- apply1 :: [Int] -> [Int]
-- apply1 = zipWith ($) [(+) 2, (-) 2, (*) 2, (`div` 2)]
    -- where 
        -- map1 (f, v) = f v
        -- (t -> t1) -> t -> t1
        -- $ :: (a -> b) -> a -> b
